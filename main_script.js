...
(function () {
    ...
    function _init() {
      ...
      if ($settings.indexUrl == window.location.href) {
        console.log('TRUE RELOAD AFTER 3 SEC')
        setTimeout(() => {
  ...
  */
       ...
       // Вибрати між myObjectStringLaLiga, myObjectStringCL
        ...
        let sessionInfo = _requestAjaxData(
          `${$settings.indexUrl}channels-api/v1/catalog/sessions/` +
            session_id
        );
  ...
  function reserveTickets(session, ticketObj) {
    ...
    return _requestAjaxData(
      `${$settings.indexUrl}channels-api/v1/checkout/seats`,
      {
  ...
  function getObChannelId() {
    ...
    if ($settings.obChannelId) {
      ...
      xhr.open(
        "GET",
        `${$settings.indexUrl}channels-api/v1/init-config/${pageType}`,
        false
      );
      try {
        xhr.send(null);
